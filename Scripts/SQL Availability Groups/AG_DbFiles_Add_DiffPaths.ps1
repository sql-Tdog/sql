...
ALTER AVAILABILITY GROUP AGname ADD DATABASE TestAG


ALTER AVAILABILITY GROUP AGname MODIFY REPLICA ON N'serverName' WITH (SEEDING_MODE = AUTOMATIC);


--remove database from the AG, putting it into restoring mode on the secondary nodes
ALTER AVAILABILITY GROUP AGname REMOVE DATABASE TestAG
...
ALTER  DATABASE TestAG SET HADR AVAILABILITY GROUP = AGname
