select S.enabled,
	   ...
	   JS.last_run_date,
	  avg(H.run_duration) [AvgDuration sec],
       CASE(SS.freq_type)
            WHEN 1  THEN 'Once'
            WHEN 4  THEN 'Daily'
...
left join msdb.dbo.sysjobsteps js ON S.job_id=js.job_id
left join msdb.dbo.sysjobhistory H on S.job_id=H.job_id
where s.name='DatabaseBackup - USER_DATABASES - LOG'
GROUP BY S.enabled, S.Name, SS.name, last_run_date, SS.freq_type, freq_subday_type, SJ.next_run_date, freq_recurrence_factor,
	SJ.schedule_id, freq_interval, SS.schedule_id, active_start_time, freq_subday_interval, next_run_time, s.job_id